  ...
  io.on('connection', function(socket){
    ...
    socket.on('join', function(roomName,username){
      return new Promise(async (resolve,reject) => {
        ...
        try {
          await helper.rpush(roomName, users_s, username);
          socket.join(roomName);
          io.to(roomName).emit('joinCompleted', username);
        } catch(e){
          socket.emit('joinCompleted', username, "Join failed due to : " + e);
        }
      }, (err) => {
        socket.emit('joinCompleted', username, err);
      });
    });
    ...
    socket.on('leave', function(roomName,username){
      return new Promise(async (resolve,reject) => {
        ...
        try {
          await helper.lrem(roomName, users_s, username);
          io.to(roomName).emit('leaveCompleted', username);
          socket.leave(roomName);
        } catch(e){
          socket.emit('leaveCompleted', username, "Leave failed due to : " + e);
        }
      }, (err) => {
        socket.emit('leaveCompleted', username, err);
      });
    });
    ...
    socket.on('messages', async function(roomName, username, rangeSize){
